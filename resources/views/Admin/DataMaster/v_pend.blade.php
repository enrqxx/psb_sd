 ...
 @section('modal')
         ...
         <div class="modal-dialog">
             <div class="modal-content">
                 <div class="modal-header modal-colored-header bg-success">
                     <h4 class="modal-title" id="success-header-modalLabel">Tambah Jenis Pendidikan</h4>
                     <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
                 </div>
                 <form id="form-tambah" enctype="multipart/form-data">
                     {{ csrf_field() }}
                     <div class="modal-body">
                         <label for="inputCity" class="form-label">Jenis Pendidikan</label>
                         <input type="text" class="form-control" id="inputCity" name="nama_pendidikan" required>
                     </div>
     ...
     <div id="edit-data" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="success-header-modalLabel"
         ...
         <div class="modal-dialog">
             <div class="modal-content">
                 <div class="modal-header modal-colored-header bg-success">
                     <h4 class="modal-title" id="success-header-modalLabel">Edit Jenis Pendidikan</h4>
                     <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
                 </div>
 ...
 @section('js')
     <script type="text/javascript">
         ...
         function edit(id) {
             const pendidikan = list_pend[id]
             $('#edit-data').modal('show');
             $("#edit-data [name='id']").val(id)
             $("#edit-data [name='nama_pendidikan']").val(pendidikan.nama_pendidikan)
         }
         ...
         function tambah_pendidikan() {
             ...
             const url = "{{ url('tambah_pendidikan') }}";
             $.ajax({
                 url,
                 method: "POST",
                 data: form.serialize(),
         ...
         function hapus(id) {
             ...
             if (c === true) {
                 $.ajax({
                     url: "{{ url('delete_pendidikan') }}?id=" + id,
                     method: "POST",
                     data: {
                         _token: '{{ csrf_token() }}'
                     },
